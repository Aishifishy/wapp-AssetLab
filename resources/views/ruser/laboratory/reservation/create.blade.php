            ...
            @if(!$currentTerm)
                <div class="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 mb-6">
                    <div class="flex">
                        ...
                        <p>Reservations aren't available right now - we need an active academic term first. Please check back later! 📚</p>
                    </div>
                </div>
            @else
                        ...
                        <div class="mb-6 bg-blue-100 border-l-4 border-blue-500 text-blue-700 p-4">
                            <div class="flex">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm">Please bring your facility reservation request form to ITSO before your reservation time. Thanks! 📝</p>
                                </div>
                            </div>
                        </div>
                    @endif
                            ...
                            <div class="mt-2">
                                <div class="time-picker-container">
                                    ...
                                    <div class="flex items-center justify-between mb-4 p-3 bg-white rounded-lg border-2 border-gray-200 shadow-sm">
                                        <div class="text-sm">
                                            <span class="font-medium text-gray-700">Your Reservation Time:</span>
                                            <span id="selected-time-display" class="ml-2 text-gray-500">Choose your start and end times below</span>
                                        </div>
                                    ...
                                    <div class="space-y-3">
                                        <div class="text-sm font-medium text-gray-700 mb-3 flex items-center">
                                            ...
                                            Choose start time, then end time:
                                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleTimeSlotClick(slotElement) {
        ...
        if (selectionState.mode === 'start' || clickedTime === selectionState.startTime) {
            ...
            showTimeSelectionSuccess(`Great! You've chosen ${formatTime12Hour(clickedTime)} as your start time. Now pick your end time! 👍`);
            
        } else if (selectionState.mode === 'end') {
            // Validate end time selection
            if (!selectionState.startTime) {
                showTimeSelectionError('Let\'s pick your start time first! 🕐');
                return;
            }
            
            if (clickedTime <= selectionState.startTime) {
                showTimeSelectionError('Your end time needs to be later than your start time! 😊');
                return;
            }
            ...
            showTimeSelectionSuccess(`Perfect! Your reservation time is set: ${formatTime12Hour(selectionState.startTime)} - ${formatTime12Hour(clickedTime)} ✨`);
    ...
    function updateTimeDisplay() {
        if (!selectionState.startTime) {
            selectedTimeDisplay.innerHTML = '<i class="fas fa-clock mr-1 text-gray-400"></i>Choose your start time from the options below';
            clearTimeSelection.classList.add('hidden');
            return;
        }
        
        if (!selectionState.endTime) {
            const duration = 'Pick end time';
        ...
        timeSlots.forEach(slot => {
            ...
            if (selectionState.startTime === slotTime) {
                ...
                if (slotTime > selectionState.startTime) {
                    ...
                    slot.title = `Choose end time: ${formatTime12Hour(slotTime)}`;
                } else {
                    updateSlotStyling(slot, 'disabled');
                    slot.title = `This time slot isn't available - please choose a time after ${formatTime12Hour(selectionState.startTime)}`;
                }
            } else {
                slot.title = `Choose ${selectionState.mode === 'start' ? 'start' : 'end'} time: ${formatTime12Hour(slotTime)}`;
            }
        });
    }
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            if (!startTime || !endTime) {
                e.preventDefault();
                showTimeSelectionError('Don\'t forget to pick both your start and end times! 😊');
                return false;
            }
            ...
            if (startHour < 7 || endHour > 21) {
                e.preventDefault();
                showTimeSelectionError('Please choose times within our office hours (7:00 AM - 9:00 PM). Thanks! 🕐');
                return false;
            }
